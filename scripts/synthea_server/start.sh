...
echo "Waiting for synthea_server to be ready..."
until curl -s -o /dev/null -w "%{http_code}" http://localhost:8002/ | grep -q "200"; do
  echo "Server is not ready yet. Retrying in 5 seconds..."
  sleep 5
done
echo "Server is up and ready for requests! You can access it at http://localhost:8002"