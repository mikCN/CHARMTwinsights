...
hapi-down:
	@ # only run these if there are containers to stop/remove
	@ # (otherwise will error out)
	@if [ -n "$(shell docker ps -q --filter ancestor=hapiproject/hapi:latest)" ]; then \
		docker stop $(shell docker ps -q --filter ancestor=hapiproject/hapi:latest); \
	fi
	@if [ -n "$(shell docker ps -a -q --filter ancestor=hapiproject/hapi:latest)" ]; then \
		docker rm $(shell docker ps -a -q --filter ancestor=hapiproject/hapi:latest); \
	fi

# show the hapi server logs
hapi-logs:
	@docker logs $(shell docker ps -q --filter ancestor=hapiproject/hapi:latest) --tail 500
